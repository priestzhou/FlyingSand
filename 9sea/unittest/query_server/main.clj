...
(suite "query result"
       (:testbench
         (fn [test]
           (let [
                 ...
                 result2 {:status "succeeded" :end-time (to-long (time/today-at 11 57))}
                 result3 {:status "failed" :end-time (to-long (time/today-at 11 50))}
                ]
             (shark/add-result 1 result1)
             (shark/add-result 2 result2)
             (shark/add-result 3 result3)
            )
           (test (to-long (time/today-at 12 00)) (* 5 60 1000))
          )
       )
       (:fact query:result:test-result-clear
              ...
              :eq
              (fn [_ _]
                [nil {:end-time 1384430220000, :status "succeeded"} nil]
              )
        )
)
